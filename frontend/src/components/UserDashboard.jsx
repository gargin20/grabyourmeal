...
function UserDashboard() {
  ...
  const handleFilterByCategory = (category) => {
    if (category == "All") {
      ...
      const filteredList = itemsInMyCity?.filter(i => i.category?.toLowerCase() === category.toLowerCase())
  ...
  return (
    <div className='w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto'>
      ...
      {searchItems && (searchItems.items?.length > 0 || searchItems.shops?.length > 0) && (
        <div className='w-full max-w-6xl flex flex-col gap-5 items-start p-5 bg-white shadow-md rounded-2xl mt-4'>
          <h1 className='text-gray-900 text-2xl sm:text-3xl font-semibold border-b border-gray-200 pb-2'>
            Search Results
          </h1>

          {searchItems.shops?.length > 0 && (
            <div className='w-full'>
              <h2 className='text-xl font-semibold text-gray-700 mb-3'>Shops</h2>
              <div className='w-full flex flex-wrap gap-4'>
                {searchItems.shops.map((shop, index) => (
          ...
          {searchItems.items?.length > 0 && (
            <div className='w-full'>
              <h2 className='text-xl font-semibold text-gray-700 mb-3 mt-4'>Food Items</h2>
